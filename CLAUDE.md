...
- **다음 (품질)** ① **prob_23류 잔여 진단(측정됨)**: 순서 교란 60%로도 greedy 구성을 못 이기고 LNS 이웃에 강개선이 없음 — tardiness가 시퀀싱-구속(한 블록을 당기면 다른 블록이 밀리는 제로섬). 다음 레버 = 온도 수락(SA식 일시 악화 허용) 또는 시간창 통째 destroy(혼잡 구간의 전 블록 제거 후 재패킹). ② prob_40이 이번 run −30.6%(15.1M vs 12.99M) — 단일 관측, 대형 인스턴스 잔여 분산 여부 재관측 필요. ③ 머신 노이즈 주의: 장시간 벤치 후 스로틀링으로 wall 측정 ±40% 변동 — 속도 비교는 같은 세션 연속 측정으로.
