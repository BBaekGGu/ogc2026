...
- **다음 (품질)** ① **구성 재시작(최우선)**: LNS가 정체하는 tardiness-지배 혼잡 인스턴스(prob_23/26류)에서 남는 시간에 순서 교란(EDD±노이즈) 재구성 3~5회 → 최고 시작점 채택. prob_23은 L1이 8~15s라 TL=300 기준 여유 충분. ② LNS tardy 심층화(시간창 통째 destroy 등)는 재시작 효과 확인 후. ③ 머신 노이즈 주의: 장시간 벤치 후 스로틀링으로 wall 측정 ±40% 변동 관측됨 — 속도 비교는 같은 세션 연속 측정으로.
